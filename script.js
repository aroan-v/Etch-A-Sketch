...
const buttons = document.querySelectorAll(
  ".shading-button, .rainbow-button, .clear-button, .erase-button, .erase-shader-button, .brush-button"
);
...
function opacityShades(div) {
  ...
  rgbaColor = `${hexToRgbA(hexColor).slice(
    0,
    indexOfLastComma
  )},${currentAlpha})`; // Slice the rgbaColor string up to the last comma and replace with the new alpha value
...
function eraserShader(div) {
  ...
  if (currentOpacity === 1) {
    ...
    rgbaColor = `${div.style.backgroundColor.slice(
      0,
      indexOfLastComma
    )},${currentAlpha})`; // Slice the parenthesis from the RGB color value and concatenate the new alpha value
  ...
  // Eraser Shader works by decrementing the alpha values.
    ...
    rgbaColor = `${div.style.backgroundColor.slice(
      0,
      indexOfLastComma
    )},${currentAlpha})`; // Slice the rgbaColor string up to the last comma and concatenate the new alpha value
    div.style.backgroundColor = rgbaColor;
    div.setAttribute("data-opacity", currentAlpha); // Manually assign the current opacity to the attribute for future reference
  }
